<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="utf-8">
		<title>Coin Host</title>
		<meta name="viewport" content="width=device-width, initial-scale=1.0">
		<meta name="description" content="">
		<meta name="author" content="">

		<!-- styles -->
		<link href="css/bootstrap.css" rel="stylesheet">
		<link href="css/bootstrap-responsive.css" rel="stylesheet">
		<link href="css/base.css" rel="stylesheet">
		<link href="css/flexslider.css" rel="stylesheet">
		<link href="css/prettyPhoto.css" rel="stylesheet">
		<link href="css/style.css" rel="stylesheet">
		<link href="css/sequence.css" rel="stylesheet">
		<link href="css/YTPlayer.css" rel="stylesheet">
		
		<!-- font awesome -->
        <link href="css/font-awesome.min.css" rel="stylesheet" type="text/css" media="screen">

		<!-- HTML5 shim, for IE6-8 support of HTML5 elements -->
		<!--[if lt IE 9]>
		  <script src="http://html5shim.googlecode.com/svn/trunk/html5.js"></script>
		<![endif]-->

		<!-- Fav and touch icons -->
		<link rel="apple-touch-icon-precomposed" sizes="144x144" href="ico/apple-touch-icon-144-precomposed.html">
		<link rel="apple-touch-icon-precomposed" sizes="114x114" href="ico/apple-touch-icon-114-precomposed.html">
		<link rel="apple-touch-icon-precomposed" sizes="72x72" href="ico/apple-touch-icon-72-precomposed.html">
		<link rel="apple-touch-icon-precomposed" href="ico/apple-touch-icon-57-precomposed.html">
		<link rel="shortcut icon" href="ico/favicon.html">
	</head>

<body>			
<!-- Image Back ground and pattern Back ground  -->
   
      <div id="pattern_bg"></div> 
    <!--   <img src="images/large/bg6.jpg" alt="" id="background" /> -->


<!-- Main header -->
<div id="main_wrapper">
	<div class="main_wrapper_inner">
	  
	  <div class="header_top_second  navbar-fixed-top">

        <div class="container header_top_second_inner">
		 <div class="row">
		   <div class="span12">
     <!-- Header -->
		    <div class="header_top_inner1">
			     <div class ="toad_wrapper">					 
					<div class="align_left logo"><a href="#"><img src="images/logo.png" alt="" /></a></div> 	
				 </div>
				 <div class="main_menu">
				        <!-- mobile menu --> 
				        <button type="button" class="btn btn-navbar" data-toggle="collapse" data-target=".nav-collapse">
						  <i class="icon-reorder"></i>
					    </button>						
						<!-- desktop menu -->
					<div class="nav-collapse collapse">
						<nav id="dropdown">
							<ul class="sf-menu clearfix nav">								
								<li><a href="#services" class="trigger"><span>Home</span></a></li>	
                                <li><a href="#product" class="trigger"><span>Product</span></a></li>								
								<li><a href="#create" class="trigger"><span>Create</span></a></li>
                                <li><a href="http://ec2-54-186-27-43.us-west-2.compute.amazonaws.com:2750/"><span>Blockchain</span></a></li>
                                <li><a href="#"><span>Manual</span></a></li>								
								<li><a href="#" class="trigger"><span>Support</span></a>
                                	<ul>
										<li><a href="https://btcft.com/forum/forum">forum</a></li>
										<li><a href="http://www.btcft.com/">social</a></li>
									</ul>
                                </li>						
							</ul>
						</nav>
                    </div>						
                 </div>						
				 <!-- end navigation -->
		    </div>
			
			</div>
		   </div>
        </div>
       <div class="clear"></div>
     </div>

	
    <div class="content">		
				
	 <div class="content_wrapper">	
	  <div class="content_wrapper_inner">
	  
	 <!-- Services Block --> 
	<section id="services" class="anchor_holder"> 
	  <div class="container content_wrapper_section">
         <div class="main_title_wrapper">		
			  <div class="block_tile_wrapper">	 
			   <h1 class="block_tile_1">ALTCOIN CREATION HAS NEVER BEEN EASIER</h1>
			    <div class="icon_separator"></div>
			    <h2 align="center">Create your own altcoin and start mining today!</h2>
			    <p align="center">With CoinCreator you can skip the time-consuming setup and start to outmatch the current crypto currencies with your own coin.</p>
			    <h6 class="block_tile_2">&nbsp;</h6>
			  </div>		 
			<div class="clear"></div>		
		 </div>	  
		 				 
		 </div>  <!-- End content wrapper section -->		 
	 <div class="content_wrapper_gray">
 <div class="container content_wrapper_section2"> 
    <div class="main_title_wrapper">		
			  <div class="block_tile_wrapper">	 
			   <h1 class="block_tile_1"> Why Should You Choose Us? </h1>
			    <div class="icon_separator"></div>
			   <h6 class="block_tile_2"> Lorem ipsum dolor sit amet, Non sed sequi rerumar quasi repellat eum <span class="color_hi">earum praesentium</span> totaral Quia, enim ad minim veniam <br>voluptas eaque anim uis aute senectus et netus et malesuada fames ac turpis
			   </h6>
			  </div>		 
			<div class="clear"></div>		
	</div>	
    <div class="container">
	<div class="row">        							    
	  <div class="service_pa2"> 	   					                			 		
        <div class="service2_wrapper_holder">
		    <div class="span6">        
                   <div class="service_wrapper3">
                       <img src="images/icons/se1.png" alt="" class="scale-with-grid"/> 
				        <div class="service_wrapper_inner3">   	  	
				            <h4><a href="#"> make a lot of money </a></h4>
							<div class="sub_dis2"> 100k will be yours just saved </div>
						    <p> sed diam nonumy eirmod tempor invid unt ut labore et dolore magna adipiocar scali dolore magna adipiocar scing consect etuer ali stibu</p>					                        						  
					    </div> <!-- End service wrapper inner -->
                      <div class="clear"></div>						
			   	   </div> <!-- End service wrapper -->
                   		   
                   <div class="service_wrapper3">   
				     <img src="images/icons/se2.png" alt="" class="scale-with-grid"/> 
				      <div class="service_wrapper_inner3"> 	      	  	
				            <h4><a href="#"> save much time </a></h4>
							<div class="sub_dis2"> we spend short time to work </div>
						    <p> sed diam nonumy eirmod tempor invid unt ut labore et dolore magna adipiocar scali dolore magna adipiocar scing consect etuer ali stibu</p>	
					  </div> <!-- End service wrapper inner --> 
					 <div class="clear"></div>
			   	   </div> <!-- End service wrapper -->

                   <div class="service_wrapper3"> 
				      <img src="images/icons/se3.png" alt="" class="scale-with-grid"/> 
					  <div class="service_wrapper_inner3">       	  	
				            <h4><a href="#"> 24/7 support ever </a></h4>
							<div class="sub_dis2"> 3000+ users trust us </div>
						    <p> sed diam nonumy eirmod tempor invid unt ut labore et dolore magna adipiocar scali dolore magna adipiocar scing consect etuer ali stibu</p>						
					  </div> <!-- End service wrapper inner --> 
					 <div class="clear"></div>
			   	   </div> <!-- End service wrapper -->
		
			</div> 
            <div class="span6">   
                <div class="center_image">  
                      <img src="images/large/iphone_v.png" alt="" class="scale-with-grid"/> 
                       <span class="clear"></span>					  
                </div>	    
			</div> 
			
     <div class="mission_wrapper">
       <div class="count_wrapper clearfix countto">

           <div class="span3">
              <div class="number_counter" data-perc="179">
			    <div class="count_border"></div> 
				<p>Up to</p>
				<div class="number_count_holder">                     				
				   <div class="number_count blue">20</div><p class="sub_num">Gb/s</p>
                </div>  				   
				<p class="number_text">data transfer bandwidth</p>
			  </div>
		   
           </div>
		   
		   <div class="span3">
              <div class="number_counter" data-perc="45">
			    <div class="count_border"></div>
				<p>Daisy chain</p>
				<div class="number_count_holder">                     				
				   <div class="number_count blue">36</div>
                </div>  				   
				<p class="number_text">devices</p>
			  </div>
		   
           </div>
		   
		   <div class="span3">
              <div class="number_counter" data-perc="77">
			    <div class="count_border"></div>
				<p>Up to</p>
				<div class="number_count_holder">                     				
				   <div class="number_count blue">25</div><p class="sub_num">x</p>
                </div>  				   
				<p class="number_text">faster than FireWire 800</p>
			  </div>		   
           </div>
		   
		   <div class="span3">
              <div class="number_counter" data-perc="57">
			    <div class="count_border"></div>
				<p>Connect up to</p>
				<div class="number_count_holder">                     				
				   <div class="number_count blue">40</div><p class="sub_num">K</p>
                </div>  				   
				<p class="number_text">for displays</p>
			  </div>
		   
           </div>
		   
       </div>	   
	 </div><!-- End client h inner -->
	 
          <span class="clear"></span>
		</div>   					   	    				
			<div class="clear"></div> 				  
      </div>
	
	 
    <div class="clear"></div>	

	</div>  <!-- End port wrapper -->
    </div>
   </div>
   <div class="clear"></div>
</div> 	 
<!-- End Services Block --> 

</section>

<!-- Start Product block --> 
<section id="product" class="anchor_holder">	 

<div class="coop_wrapper">	
    <div class="coop_control_wrapper">		
	     <div class="main_title_wrapper">		
			  <div class="block_tile_wrapper">	 
			   <h1 class="block_tile_1 color_black"> Coin table</h1>
			    <div class="icon_separator"></div>
                <h6 class="block_tile_2"> Lorem ipsum dolor sit amet, <span class="color_hi">consectetur </span>adipisicing elit. Non sed sequi rerumar quasi repellat eum <span class="color_hi">earum praesentium</span> totam! <br>
                                         Quia, voluptas eaque anim uis aute irure dolor in reprehenderit in voluptate velit esse 
			   </h6>			    
			  </div>		 
			<div class="clear"></div>		
		 </div>
	  
        <div class="fun_wrapper_holder">	
           <div class="container">
		     <div class="row">					   		   
                <div class="span12">

			 <div class="pricing">
            
            	<div class="pricing_control">
                	<ul>
                    	<li class="price_header">FreeCoin(0.00 btc)</li>
                        <li class="price">$0.00<span>/ 7 Days Trial</span></li>
			<li class="item">FreeCoin is a simple cryptocurrency derived from Bitcoin. FreeCoins are living on another blockchain and has limited amount of coins. And you can have a trial of most of the services provided by us. Freecoin is an excellent choice for testing purpose.</li>
                        <li class="item">Initialized with 1000 coins</li>
                        <li class="item">Free Trial With Wallet</li>
                        <li class="item">Free Trial With CFOS</li>
			<li class="item"> 	 X 	 </li>
                        <li class="item">	 X 	 </li>
			<li class="item">	 X 	 </li>
			<li class="item">	 X 	 </li>
			<li class="item">	 X 	 </li>
			<li class="item">	 X 	 </li>
                        <li class="buy"><a href="#create"><button class="button">Get it now</button></a></li>
                    </ul>
                </div>
                
                <div class="pricing_control">
                	<ul>
                    	<li class="price_header">Standard (0.02 btc)</li>
                        <li class="price">$9.90<span>/ Month</span></li>
			<li class="item">You can freely customize your coin and have access to Crypto Financial Operating System(CFOS), which means that you can easily share your coins with other people, transfer it from your wallet, trade it and build your own trust-based financial future.</li>
                        <li class="item">Initialized with 100,000 coins</li>
                        <li class="item">Standard Silverlight Wallet</li>
                        <li class="item">Standard Access to CFOS</li>
			<li class="item">Customized Name and Logo</li>
                        <li class="item">Unique Merkle Hash</li>
			<li class="item">Unique Genesis block</li>
			<li class="item">X</li>
			<li class="item">X</li>
			<li class="item">X</li>
                        <li class="buy"><a href="#create"><button class="button">Get it now</button></a></li>
                    </ul>
                </div>
                
                <div class="pricing_control">
                	<ul>
                    	<li class="price_header">Premium (0.10 btc)</li>
                        <li class="price">$49.9<span>/ Month</span></li>
			<li class="item">In ultimate version, full access to all of our premium services is guaranteed. Additionally, you would receive a chance for learning bitcoin source code, testing experimental projects and working with us in our Github like Cryptocurrency Innovation Lab(CIL).</li>
                        <li class="item">Initialized with 100,000 coins</li>
                        <li class="item">Premium Silverlight Wallet</li>
                        <li class="item">Premium Access to CFOS</li>
			<li class="item">Customized Name and Logo</li>
                        <li class="item">Unique Merkle Hash</li>
			<li class="item">Unique Genesis block</li>
			<li class="item">Bitcoin Derivative Education</li>
			<li class="item">Experimental Projects Testing</li>
			<li class="item">Laboratory Accesible</li>
                        <li class="buy"><a href="#create"><button class="button">Get it now</button></a></li>
                    </ul>
                </div>
            
            </div>
				
                </div>				
			 </div>
		    </div><!-- End contain -->
		</div>
    </div>  		  
  <span class="clear"></span> 
</div>
<!-- End Price block -->

 </section>
<!-- End Price block -->  


 <!-- Team Block --> 
	<section id="create" class="anchor_holder"> 

	  
 <div id="coop_background_4" class="bg_parallax">			    
	        <div class="contact_wrapper">
			 <div class="container">
				<div class="row">		 
				  <div class="span6">
					<div class="background_gray_con">
					 <h5>Create Coins Now</h5>
					 <p> consectetur adipisicing elit. Non sed sequi rerumar quasi repellat eum earum praesentium totam vestibum tortor quam, feugiat ultricies euismod indor pharetra orliver eget vitae.  </p>
					 
					<form class="contact_form contact_form_h" action="contact.php" style="margin-bottom: 10px;">
					
						<div class="input_holder">
							<input class="field-name" type="text" placeholder="Name (required)">
						</div>
						<div class="input_holder">
							<input class="email_field" type="text" placeholder="Email (required)">
						</div>
						<div class="input_holder">
							<input class="subject_field" type="text" placeholder="Coin Abbr(required)">
						</div>
						<div class="textarea_holder">
							<textarea class="comment_field" placeholder="Message"></textarea>
						</div>
						<a href="#" class="button">Create Coins</a>
						
					</form>
                   <div class="clear"></div>					
				  </div> 
			   </div>
			   
			   <div class="span6">
			       <div class="contact_side">
				     <p>   dolor sit amet, consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna aliquyam erat, sed diam voluptua. 
					  rebum sed diam nonumy eirmod tempom invid ut labore et dolore magna aliquyam erat, sed diam voluptua. </p>
					  <h1> WE LOVE TO SUPPORT YOU </h1>
					  
					  <div class="contact_detail">
					     <div class="contact_circle_holder">
						    <div class="c_d_text">Address :  Wall Street, New York </div><div class="contact_circle"><i class="icon-map-marker"></i></div>
					     </div>
						 <div class="contact_circle_holder">
						    <div class="c_d_text">Phone : +1(111)1-111-111 , +1(111)1-111-111 </div><div class="contact_circle"><i class="icon-phone"></i></div>
					     </div>
						 <div class="contact_circle_holder">
						    <div class="c_d_text">Fax : +1(111)1-111-111 , +1(111)1-111-111 </div><div class="contact_circle"><i class="icon-print"></i></div>
					     </div>
						 <div class="contact_circle_holder2">
						    <div class="c_d_text">Mail : bitcoincds@gmail.com </div><div class="contact_circle"><i class="icon-envelope"></i></div>
					     </div>
					  </div>
					  
				   </div>
			   </div>
			   
		     </div>		 
			</div>	
	 </div>
</div>
</section>
<!-- End Contact Block --> 
 
		   
		  </div> 	 <!-- End Content wrapper inner -->  
        </div> 	 <!-- End Content wrapper -->	
    </div>	<!-- End content -->
	
	
  	<!-- Footer -->

         <footer id="footer">	   
			<div class="container">
			  <div class="footer_inner"> 
			  <div class="row">
			        
				 <div class="span5">	
                     <div class="footer_left">
					     <div class="social_wrapper_ft">  
							<a href="#"><img src="./images/facebook.png" alt=""/></a>
							<a href="#"><img src="./images/twitter.png" alt=""/></a>
							<a href="#"><img src="./images/skype.png" alt=""/></a>
							<a href="#"><img src="./images/youtube.png" alt=""/></a>
                            <a href="#"><img src="./images/vimeo.png" alt=""/></a>
                            <a href="#"><img src="./images/rss.png" alt=""/></a>							
						 </div>
					 </div>
				   
					  <div class="clear"></div>
                 </div>
                          
                <div class="span7">
                      <div class="footer_right">
					     <img src="images/logo2.png" alt=""/>
						 <div class="clear"></div>
					     Site by <a href="http://www.btcft.com/">Bitcoin Future</a>
					 </div>
				<div class="clear"></div>
              </div>
			
			   </div>
			 </div>
            </div>
		 </footer> <!-- End Footer -->
		
	</div> <!-- End main_wrapper_inner -->	   
</div> <!-- End main_wrapper -->	
	
	
<!-- jquery -->
<script src="js/jquery.js" type="text/javascript"></script>
<script src="js/jquery.queryloader2.js"  type="text/javascript"></script>
<script src="js/bootstrap.js" type="text/javascript"></script>
<script src="js/jquery.tipsy.js" type="text/javascript"></script>
<script src="js/jquery.easing.1.3.js" type="text/javascript"></script>
<script src="js/jquery.flexslider-min.js" type="text/javascript"></script>
<script src="js/superfish.js" type="text/javascript"></script>
<script src="js/jquery.color.js" type="text/javascript"></script>
<script src="js/custom.js" type="text/javascript"></script>
<script src="js/jquery.appear.min.js" type="text/javascript"></script>
<script src="js/jquery.countto.js" type="text/javascript"></script>
<script src="js/jquery.cycle.all.js"  type="text/javascript"></script>
<script src="js/modernizr.custom.js"  type="text/javascript"></script>
<script src="js/jquery.fitvids.js"  type="text/javascript"></script>
<script src="js/jquery.ui.totop.js"  type="text/javascript"></script>
<script src="js/jquery.smoothscroll.js"  type="text/javascript"></script>
<script src="js/jquery.prettyPhoto.js"  type="text/javascript"></script>
<script src="js/jquery.nav.js"  type="text/javascript"></script>
<script src="js/jquery.cbpQTRotator.min.js"  type="text/javascript"></script>
<script src="js/jquery.isotope.js" type="text/javascript"></script>

<!-- Bg video -->

<script src="js/jquery.mb.YTPlayer.js" type="text/javascript"></script>


<!-- Big slider -->
<script src="js/jquery.sequence.js" type="text/javascript"></script>
<script src="js/sequence_custom.js" type="text/javascript"></script>

<script src="http://maps.googleapis.com/maps/api/js?sensor=false" type="text/javascript"></script>
<script src="js/gmap3.js" type="text/javascript" ></script> 

<!-- google webfont font replacement  -->
<link href='http://fonts.googleapis.com/css?family=Open+Sans:400,400italic,700,700italic,300,300italic' rel='stylesheet' type='text/css'>
<link href='http://fonts.googleapis.com/css?family=Raleway:400,500,600,700,800,900,300' rel='stylesheet' type='text/css'>
	</body>
</html>
